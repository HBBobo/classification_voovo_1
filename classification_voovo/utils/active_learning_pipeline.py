...
async def retrain_student_model_async() -> Tuple[Optional[Any], Optional[List[str]]]:
    ...
    texts = [item['text'] for item in full_dataset if item.get("text") and item.get("text").strip()]
    
    if not texts:
        print("Error during retraining: No valid text samples found in the dataset.")
        return None, None

    topic_labels = sorted(full_dataset[0]['scores'].keys())
    y_data = np.array([[item['scores'].get(label, 0.0) for label in topic_labels] for item in full_dataset if item.get("text") and item.get("text").strip()])
...
async def run_pipeline_async():
    ...
    for i, para_info in enumerate(all_paragraphs_with_origin):
        ...
        if not para_text or not para_text.strip():
            print(f"\nSkipping empty paragraph discovered from {origin}.")
            continue
